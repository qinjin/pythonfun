...
class Shape():
    ...
    def can_move(self, board, dx, dy):
        ...
        for a_block in self.blocks:
            if a_block.can_move(board,dx,dy):
                count+=1
        if count==4:
            return True
        else:
    ...
    def can_rotate (self, board):
        ...
        count=0
        for a_block in self.blocks:
            if 0<=a_block.x<board.width and 0<=a_block.y<board.height:
                count+=1
        if count==4:
            return True
        else:
    ...
    def rotate(self, board):
        ...
        '''    
        ro_dir=self.get_rotation_dir()
        
        for a_block in self.blocks:
            print 'origin (x,y):',a_block.x, a_block.y
            new_x=self.blocks[1].x-ro_dir*self.blocks[1].y+ro_dir*a_block.y
            new_y=self.blocks[1].y+ro_dir*self.blocks[1].x-ro_dir*a_block.x
            print 'new (x,y):', new_x, new_y
            print '*'

            a_block.move((new_x-a_block.x),(new_y-a_block.y))
...
class Board():
    ...
    def add_shape(self, shape):
        ...
        '''
        for a_block in shape.get_blocks() :
            self.grid[(a_block.x,a_block.y)]=a_block
    ...
    def delete_row(self, y):
        ...
        '''
        for a_block in self.grid.values():
            if a_block.y==y:
                del [(a_block.x,a_block.y)]
                a_block.undraw()
    ...
    def is_row_complete(self, y):        
        ...
        for a_block in self.grid.values():
            if a_block.y==y:
                blocks_rowY.append(a_block)

        if len(blocks_rowY)!=self.width:
            return False
        else:
            return True        
    ...
    def move_down_rows(self, y_start):
        ...
        for a_block in self.grid.values():
            ...
            if y<=y_start:
                del [(a_block.x,a_block.y)]
                a_block.move(0,1)
                self.grid[a_block.x,a_block.y+1]=a_block
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        shape=self.SHAPES[random.randint(0,6)]
        a_shape=shape(Point(int(self.BOARD_WIDTH/2),0))
        return a_shape
    ...
    def do_move(self, direction):
        ...
        '''
##        print direction
        self.x=self.DIRECTION[direction][0]
        self.y=self.DIRECTION[direction][1]
        
        
        if self.current_shape.can_move(self.board,self.x,self.y):
            self.current_shape.move(self.x,self.y)
            return True
        else:
            if direction=='Down':
                self.board.add_shape(self.current_shape)
                for a_block in self.board.grid.values():
                    y=a_block.y
...
##        print key
        ...
        if key=='Left':
            ...
            direction='Down'
            self.do_move('Down')
            while self.current_shape.can_move(self.board,self.x,self.y):
                self.do_move('Down')
            self.board.add_shape(self.current_shape)
            self.current_shape = self.create_new_shape()
            self.board.draw_shape(self.current_shape)
